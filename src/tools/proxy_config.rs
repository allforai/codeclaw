...
impl ProxyConfigTool {
    ...
    fn parse_scope(raw: &str) -> Option<ProxyScope> {
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            "codeclaw" | "zeroclaw" | "internal" | "core" => Some(ProxyScope::Zeroclaw),
    ...
    async fn handle_set(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(scope_raw) = args.get("scope") {
            ...
            proxy.scope = Self::parse_scope(scope).ok_or_else(|| {
                anyhow::anyhow!("Invalid scope '{scope}'. Use environment|codeclaw|services")
            })?;
        }
...
impl Tool for ProxyConfigTool {
    ...
    fn description(&self) -> &str {
        "Manage CodeClaw proxy settings (scope: environment | codeclaw | services), including runtime and process env application"
    }
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "scope": {
                    "type": "string",
                    "description": "Proxy scope: environment | codeclaw | services"
                },
                "http_proxy": {
                    "type": ["string", "null"],
