...
/// Tokens are cached to `~/.config/codeclaw/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "codeclaw")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("codeclaw-copilot-{user}"))
            });
