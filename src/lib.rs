...
Examples:
  ...
  codeclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  codeclaw channel bind-telegram 123456789")]
    BindTelegram {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `CodeClaw` workspace
    Openclaw {
...
Examples:
  ...
  codeclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  codeclaw cron add-every 60000 'Ping heartbeat'     # every minute
  codeclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  codeclaw cron once 2h 'Follow up on deployment'
  codeclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  codeclaw cron update <task-id> --expression '0 8 * * *'
  codeclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  codeclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  codeclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  codeclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash CodeClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash CodeClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  codeclaw peripheral flash
  codeclaw peripheral flash --port /dev/cu.usbmodem12345
  codeclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash CodeClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
