...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 CodeClaw Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `codeclaw doctor` again.");
    }
...
pub async fn run_models(
    ...
    println!("🩺 CodeClaw Doctor — Model Catalog Probe");
...
pub fn run_traces(
    ...
    println!();
    println!("Use `codeclaw doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `codeclaw onboard` to set one up",
        ));
    }
